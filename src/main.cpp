...
void usage() {
    ...
    << "  -k                        Indicate that the input SAM/BAM files were "
    << "generated by kallisto genomebam. Required for BAM files." << endl
    << "  -R                        Indicate that the input SAM/BAM files are "
...
#if READ_DIST
        {"mapped", required_argument, 0, 'm'},
#endif
        {"check-gff", no_argument, no_argument, 'G'}
    };
    int opt_index = 0;
    string stringopts = "g:S:o:Ut:p:e:fu:kR";
#if READ_DIST
    stringopts += "m:";
#endif
    while (true) {
        int c = getopt_long(argc, argv, stringopts.c_str(), opts, &opt_index);
        if (c == -1) { break; }
        switch (c) {
...
#if READ_DIST
            case 'm':   mapped = parseString(optarg, ",", 0); break;
#endif
...
#if READ_DIST
    for (auto file = mapped.begin(); file != mapped.end(); ++file) {
        if (!testOpen(*file, 1)) {
            cerr << "ERROR: failed to open output file " << *file << endl;
            return 1;
        }
